...
배포 후에는 Apache JMeter 를 통해 애플리케이션에 Simple HTTP Request 를 잦은 시간 동안 주기적으로 보내 배포 과정 동안 다운타임이 없는지 테스트하고 Postman을 사용하여 헬스 체크 API 요청을 보내 구 버전의 애플리케이션이 안전히 내려갔는지 확인하며 무중단 배포 후의 애플리케이션 상태를 점검합니다.
...
4. **헬스 체크**
    - 새 애플리케이션이 제대로 실행되는지 확인하기 위해 최대 50초 동안 헬스 체크를 반복합니다.
